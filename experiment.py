...
class Experiment:
    ...
    def set_path(self, destination, file_name):
        ...
        if DEBUG: print(f"Directory = {dir1}")
        return dir1 + self.experiment_name + file_name


    def save_solution(self, best_solution, best_fitness, fitness):
        """
        ...
        solution_fitness_max = best_fitness
        solution_fitness_avg = np.average(self.generation_fitness)

        self.solution_data = np.array([solution_fitness_avg, solution_fitness_max])
        ...
        if DEBUG: print(f"Path to solution = {path_to_solution}")
    ...
    def store_data_generation(self, generation, fitness):
        """
        ...
        """
        self.calculate_average_generation_fitness(fitness)
        
        # STORE DATA
        destination = "_generation_data\\"
        file_name = '_generation_data.csv'
        path_to_generation = self.set_path(destination, file_name)
        if DEBUG: print(f"Path to solution = {path_to_generation}")

        # STORE VALUES IN .CSV
        with open(path_to_generation,mode='w') as gen_fit:
            csv_writer = csv.writer(gen_fit, delimiter=',')
            csv_writer.writerow(self.generation_fitness)

        # # Add generation_fitness value to .csv file with a. generation number, b. generation_fitness
        # with open(self.experiment_name + '_generation_fitness.csv', 'w') as new_file:
        #     csv_writer = csv.writer(new_file, delimiter=',')
        #     csv_writer.writerow(self.generation_fitness) # add generation_fitness to .csv file
            
        print(f'Generation {generation} average fitness: {self.generation_fitness[generation-1]}')
